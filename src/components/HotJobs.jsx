...
function HotJobs() {
    ...
    const handleApplyNow = (job) => {
        // Handle the apply now action, e.g., redirect to an application form
        alert(`Applying for ${job.title}`);
        // You can also redirect to a specific URL or open a modal here
    };
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {jobs.length > 0 ? (
                    jobs.map(job => (
                        <div key={job._id} className="bg-white shadow-lg rounded-lg p-6 transition-transform transform hover:scale-105">
                            <div className="flex items-center mb-4">
                                <img src={job.company_logo} alt={`${job.company} logo`} className="w-16 h-16 rounded-full object-cover" />
                                <div className="ml-4">
                            ...
                            <p className="text-gray-600"><strong>Application Deadline:</strong> {job.applicationDeadline}</p>
                            ...
                            <ul className="list-disc list-inside mb-4">
                                {job.requirements.map((req, index) => (
                                    <li key={index} className="text-gray-600 border-b p-2">{req}</li>
                                ))}
                            </ul>

      
                                <p className="text-gray-600"><strong>Salary Range:</strong> {job.salaryRange.min} - {job.salaryRange.max} {job.salaryRange.currency}</p>
